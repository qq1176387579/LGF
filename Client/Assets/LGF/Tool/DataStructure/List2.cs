...
namespace LGF
{
    ...
    public class List2<T> where T : IList2Idx<T>
    {
        ...
        public void Add(T obj)
        {
            if (obj.idx > 0)
            {
                Debug.LogError($"list2 {typeof(T).Name} 已经添加了 无法在添加了");
                return;
            }

            if (InForEach)
            {
                Debug.LogError($"list2 {typeof(T).Name} 不能在ForEach时候 删除 添加");
                return ;
            }
        ...
        public void Remove(T obj)
        {
            if (obj.idx == 0)
            {
                Debug.LogError($"list2 {typeof(T).Name} 未注册过 或者 重复删除");
                return;
            }
            ...
            {
                Debug.LogError($"list2 {typeof(T).Name} 不在同一个容器里面");
                return;
            }

            if (InForEach)
            {
                Debug.LogError($"list2 {typeof(T).Name} 不能在ForEach时候 删除");
                return;
            }
