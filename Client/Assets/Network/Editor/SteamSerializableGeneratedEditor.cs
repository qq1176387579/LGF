...
namespace LGF.Editor
{
    public class SteamSerializableGeneratedEditor : EditorWindow
    {
        ...
        public static void GeneratedAll()
        {
            ...
            Assembly[] assems = new Assembly[1] { Assembly.Load("Assembly-CSharp") }; //AppDomain.CurrentDomain.GetAssemblies();
...
<GenericStreamWriteMethods>
       ...
       public static void Read_Dictionary_<MemberType>_<MemberType1>(this LGF.Serializable.LStream stream, ref Dictionary<<MemberType>, <MemberType1>> dic)
        ...
        static void Generated_RegisterNetMsgHandlingMgr()
        {
            ...
            //notRegister.Add(NetMsgDefine.Connect);
            //notRegister.Add(NetMsgDefine.FrameOpKey);

            Assembly assems = Assembly.Load("Assembly-CSharp");
            ...
            //HashSet<NetMsgDefine> notRegisterc2s = new HashSet<NetMsgDefine>();
            //notRegisterc2s.Add(NetMsgDefine.GGSYEvent);
            ...
            foreach (var item in Enum.GetValues(typeof(NetMsgDefine)))
            {
                ...
                NetMsgDefine type = (NetMsgDefine)item;
                string c2sName = $"C2S_{type.ToString()}";
                string s2cName = $"S2C_{type.ToString()}";
                var c2sype = assems.GetType(c2sName);
                var s2cype = assems.GetType(s2cName);


                if (notRegister.Contains(type))
                    continue;
        ...
        static string S2CMsgString = @"                case NetMsgDefine.<NetMsgName>: InvokeClientMsg<S2C_<NetMsgName>>(type, _stream); break;";
        static string C2SMsgString = @"                case NetMsgDefine.<NetMsgName>: InvokeServerMsg<C2S_<NetMsgName>>(type, session, _stream); break;";
